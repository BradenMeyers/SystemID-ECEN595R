...
# Interpolation helper for HoloOcean sim data, which only updates forces on change (ZOH)
...
def parse_rosbag(bag_path, auv_ns):
    ...
    if is_real and "PWM" in data:
        ...
        data["F"] = np.column_stack((data["PWM"][:, 0], f_body))  # Stack PWM times with forces
...
def solve(bag_path):
    ...
    for idx, axis in enumerate(axis_names):
        ...
        mask = np.abs(V) > 0.001    # Remove points where velocity is near 0
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: sysid_wls.py <bag_file>")
        sys.exit(1)
    solve(sys.argv[1])
